...
const actions = {
  ...
  callTrasfer({ state }) {
    ipcRenderer.send(
      "log-info",
      "transferindo ligação para o numero" + state.number
    );
        ...
        switch (e.cause) {
          case "Busy":
            ...
            Swal.fire({
              ...
              title: "Oops...",
              text:
                "Erro ao realizar ligação. chamada não autorizada ou recuzada",
            });
            break;
          case "Canceled":
          ...
          case "User Denied Media Access":
            ipcRenderer.send(
              "log-warn",
              "Ligação finalizada (microfone ou fone com problema)"
            );
            Swal.fire({
              icon: "info",
      ...
      if (state.rtcSession) {
        ...
        if (e.originator == "remote") {
          ...
          if (!state.autoatendimento) {
            ipcRenderer.send(
              "log-info",
              "Recebendo ligação de " + state.userRing
            );
            state.inCall = false;
            state.inRing = true;
            state.changer.play();
            ...
            ipcRenderer.send(
              "log-info",
              "Recebendo ligação de " + state.userRing + "(Autoatendimento)"
            );
            state.inCall = true;
            state.inRing = false;
            state.beep.play();
          ...
          state.rtcSession.on("failed", (e) => {
            ...
            if (e.cause == "Rejected") {
              ipcRenderer.send(
                "log-warn",
                "Ligação " + state.userRing + " recusada"
              );
              Swal.fire({
                icon: "warning",
